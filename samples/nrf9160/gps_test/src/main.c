...
int main(void)
{
	...
	// Init button, cheat and assume it is same device as led
	gpio_pin_configure(dev, DT_ALIAS_SW0_GPIO_PIN, GPIO_DIR_IN | GPIO_PUD_PULL_UP);
	int btn_state = 0;
	while (btn_state == 0){
		gpio_pin_read(dev,DT_ALIAS_SW0_GPIO_PIN,&btn_state);
	}
