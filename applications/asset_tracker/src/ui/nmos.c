...
static int pwm_out(u32_t pin, u32_t period_us, u32_t duty_cycle_us)
{
	...
	if (current_period_us != period_us) {
		...
		k_sleep(MAX(K_MSEC(current_period_us / USEC_PER_MSEC), K_MSEC(1)));
	}
...
int ui_nmos_init(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(nmos_pins); i++) {
		pwm_pin_set_usec(pwm_dev, nmos_pins[i].pin, DEFAULT_PERIOD_US, 0);
	}
...
int ui_nmos_write(size_t nmos_idx, u8_t value)
{
	...
	return pwm_out(nmos_pins[nmos_idx].pin, current_period_us, current_period_us * value); /* Set full or empty PWM cycle */
}
