...
The Asset Tracker demonstrates how to use the :ref:`lib_nrf_cloud` to connect an nRF9160-based board to `nRF Cloud`_ via LTE, transmit GPS and sensor data, and retrieve information about the device.
...
The LTE Link Monitor PC tool can be used to send AT commands and receive the response.
...
Button 1:
    ...
    * Toggle GPS operation (long press the button for a minimum of 10 seconds).

Switch 1 (only on nRF9160 DK):
    * Toggle to simulate orientation change (flipping) of the board.

Switch 2 (only on NRF9160 DK):
    * Set power optimization mode, see power_opt_.

On the nRF9160 DK, the application state is indicated by the LEDs.

LED 3 and LED 4:
    * LED 3 blinking: Connecting - The device is resolving DNS and connecting to nRF Cloud.
...
.. csv-table:: Thingy:91 LED states
   :header: "Color", "State"
   "White", "Connecting to network"
   "Cyan", "Connecting to nRF Cloud"
...
The Asset Tracker can run in three power modes that are configured in the Kconfig file of the application. These settings are currently only supported on the nRF9160 DK. Note that not all cellular network providers support these modes, and the granted parameters can vary between networks.
...
#. The first time you start the application, pair the device to your account:
   ...
   #. If the pairing was successful, the board and your nRF Cloud account are paired, and the device reboots.
      If the LED(s) indicate an error, check in the terminal window which error occurred.
      The device must be power-cycled to restart the pairing procedure.
   #. After reboot, the board connects to nRF Cloud.
...
#. Press button 1 to send BUTTON data to nRF Cloud.
#. Press button 1 for a minimum of 10 seconds to enable GPS tracking.
   The board has to be outdoors in clear space for a few minutes to get the first position fix.
